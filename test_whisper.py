...
def split_audio_by_vad_optimized(audio_path, min_voice_duration=3.0, aggressiveness=1):
    ...
    # 빠른 오디오 로딩
    audio_data, sample_rate = load_audio_fast(audio_path)
...
def process_file(mp3_path, output_dir, model, device, file_num, total_files):
    """단일 파일 처리 함수 - 개선된 VAD 사용"""
    print(f"\n[{file_num}/{total_files}] 파일 처리 시작: {os.path.basename(mp3_path)}")
    ...
    # 개선된 VAD로 음성 구간 나누기
    try:
        segments, sample_rate = split_audio_by_vad_optimized(mp3_path, min_voice_duration=2.0)
    except Exception as e:
        print(f"VAD 분할 실패: {e}")
        return
...
if __name__ == "__main__":
    ...
    parser.add_argument('--threads', type=int, default=1,
                      help='동시 처리 스레드 수 (VAD 최적화로 단일 스레드 권장)')
    ...
    if '*' in input_path:
        import glob
        mp3_files = sorted(glob.glob(input_path))
        if not mp3_files:
    ...
    if args.threads == 1:
        ...
        for i, mp3_path in enumerate(mp3_files, 1):
            process_file(mp3_path, output_dir, model, device, i, total_files)
    else:
        # 멀티스레드 처리
        with ThreadPoolExecutor(max_workers=args.threads) as executor:
            futures = []
            for i, mp3_path in enumerate(mp3_files, 1):
                future = executor.submit(process_file, mp3_path, output_dir, model, device, i, total_files)
